...
def health():
    ...
    return jsonify({'status': 'healthy', 'service': 'analysis-api'})
...
def health_all():
    ...
    checks = {
        'falcosidekick': {
            'url': os.environ.get('SIDEKICK_HEALTH_URL', 'http://falcosidekick:2801/healthz'),
        },
        'loki': {
            'url': os.environ.get('LOKI_HEALTH_URL', 'http://loki:3100/ready'),
        },
        'prometheus': {
            'url': os.environ.get('PROMETHEUS_HEALTH_URL', 'http://prometheus:9090/-/ready'),
        },
        'grafana': {
            'url': os.environ.get('GRAFANA_HEALTH_URL', 'http://grafana:3000/api/health'),
        },
    }
...
def analyze_page():
    ...
    Query params:
        ...
        - store: whether to store result in Loki (default: true)
        - show_mapping: whether to display obfuscation mapping (default: false)
    """
    try:
...
def history_page():
    """List all cached analyses as an HTML page."""
...
def history_detail(cache_key: str):
    """View a specific cached analysis."""
...
def api_history():
    """API endpoint: list cached analyses as JSON."""
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            h3 {{ color: #d8d9da; margin-top: 20px; }}
            ...
            .badge {{ display: inline-block; background: #3274d9; color: white; padding: 3px 8px; border-radius: 4px; font-size: 0.8em; margin-left: 8px; }}
        </style>
    </head>
    <body>
        <h1>🛡️ SIB Analysis API</h1>
        <p>AI-powered security alert analysis with privacy protection. Stack: <strong>Falco → Falcosidekick → Loki → Grafana</strong></p>
        ...
        <h3>GET /analyze <span class="badge">Web UI</span></h3>
        <p>Analyze an alert and display results (for Grafana data links).</p>
        <pre>GET /analyze?output=&lt;alert_text&gt;&amp;rule=&lt;rule_name&gt;&amp;priority=&lt;priority&gt;&amp;hostname=&lt;host&gt;</pre>
        <p>Optional params: <code>store=true|false</code> (push to Loki), <code>show_mapping=true|false</code> (show obfuscation map)</p>

        <h3>POST /api/analyze <span class="badge">JSON API</span></h3>
    ...
    "store": true
        ...
        <h3>GET /history <span class="badge">Web UI</span></h3>
        <p>Browse all cached analyses.</p>

        <h3>GET /api/history <span class="badge">JSON API</span></h3>
        <p>List cached analyses as JSON. Optional: <code>?limit=N</code> (default 50).</p>
        
        <h3>GET /health <span class="badge">JSON API</span></h3>
        <p>Simple health check for this service.</p>
...
if __name__ == '__main__':
    ...
    print(f"SIEM Analysis API starting on http://{args.host}:{args.port}")
    print(f"Stack: Falco → Falcosidekick → Loki → Grafana")
    print(f"Grafana data link: http://localhost:{args.port}/analyze?output={{alert}}")
