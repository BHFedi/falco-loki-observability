...
class Obfuscator:
    ...
    # Based on TruffleHog detectors: https://github.com/trufflesecurity/trufflehog
    PATTERNS = {
        # Network identifiers
    ...
    def _obfuscate_secrets(self, text: str) -> str:
        ...
        Critical for Falco alerts that may contain leaked credentials.
        """
        # Define secret patterns to check (most specific first)
        secret_patterns = [
    ...
    def _obfuscate_paths(self, text: str) -> str:
        ...
        def replace_path(match):
            ...
            for part in parts:
                ...
                elif part in ('home', 'var', 'tmp', 'etc', 'usr', 'opt', 'root', 'proc', 'sys', 'dev', 'bin', 'sbin', 'lib', 'lib64'):
    ...
    def obfuscate(self, text: str) -> str:
        ...
        Levels:
        - minimal: Only secrets/credentials (fastest, use when running local Ollama)
        - standard: IPs, hostnames, users, containers (recommended for shared environments)
        - paranoid: Everything except alert structure (maximum privacy)
        
        Args:
            text: Raw Falco alert output containing potentially sensitive data
...
def obfuscate_alert(alert: dict, level: str = "standard") -> tuple[dict, dict]:
    ...
    # Obfuscate output_fields if present (Falco structured data)
