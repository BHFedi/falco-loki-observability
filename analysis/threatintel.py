"""
Threat Intelligence Module — SIB Analyser Container
...
FEED_METADATA: dict[str, tuple[str, str, str]] = {
    "feodotracker":   ("Feodo Tracker",    "CRITICAL", "Active botnet C2 servers (Dridex, Emotet, QakBot, BazarLoader) — 30-day window"),
    "sslbl":          ("SSL Blacklist",     "CRITICAL", "Malware C2 IPs identified via SSL certificates (from SSLBL CSV)"),
    "threatfox":      ("ThreatFox",         "CRITICAL", "Community IOC platform — recent IP-based C2 indicators (abuse.ch)"),
...
C2_FEEDS: frozenset[str] = frozenset({"feodotracker", "sslbl", "threatfox"})
...
def enrich_alert_with_threatintel(
    alert: dict,
    obfuscation_map: Optional[dict] = None,
) -> dict:
    """
    ...
    ----------
    alert:
        Raw (pre-obfuscation) Falco alert dict. Real IPs are extracted here
        so that obfuscation tokens like [IP-EXTERNAL-1] don't defeat the lookup.
    obfuscation_map:
        The mapping dict returned by ``obfuscate_alert()``
        (shape: ``{"ips": {"1.2.3.4": "[IP-EXTERNAL-1]", ...}, ...}``).
        When provided, ``context_for_llm`` replaces every real IP with its
        obfuscation token so the LLM never sees raw IP addresses —
        preserving the privacy guarantee regardless of obfuscation level.
        When None (e.g. in standalone use), real IPs appear in the context.

    Return shape::

        {
            "threat_intel": {
                "checked_ips": [...],        # real IPs — NOT sent to LLM
                "malicious_ips": [...],      # real IPs — NOT sent to LLM
                "has_threats": bool,
                "has_c2": bool,
    ...
    # Build the reverse lookup: real_ip → obfuscation_token.
    # The obfuscation_map["ips"] dict is keyed by real IP.
    ip_to_token: dict[str, str] = {}
    if obfuscation_map:
        ip_to_token = obfuscation_map.get("ips", {})

    def _safe_ip(ip: str) -> str:
        """Return the obfuscation token for an IP, or the IP itself if no map."""
        return ip_to_token.get(ip, ip)

    lines = [f"Threat Intelligence ({len(ips)} IP(s) checked):"]
    for ip, r in results.items():
        token = _safe_ip(ip)
        if r.is_malicious:
            feed_names = ", ".join(m.display_name for m in r.matches)
            if r.in_spamhaus_cidr:
                feed_names += f", Spamhaus DROP (CIDR {r.in_spamhaus_cidr})"
            line = f"  • {token}: MALICIOUS — found in {feed_names}. Severity: {r.highest_severity}."
            if r.is_c2:
                line += " CONFIRMED C2 SERVER."
            if r.is_tor:
                line += " Tor exit node."
        else:
            line = f"  • {token}: clean (not found in any feed)."
        lines.append(line)

    if has_c2:
        lines.append(
            "  ⚠ One or more IPs are confirmed C2 servers — "
            "treat this alert as CRITICAL priority."
        )

    return {
        "threat_intel": {
            "checked_ips": ips,          # real IPs stored internally, never in LLM prompt
            "malicious_ips": malicious,  # real IPs stored internally, never in LLM prompt
            "has_threats": bool(malicious),
            "has_c2": has_c2,
            "highest_severity": highest,
            "results": {ip: r.to_dict() for ip, r in results.items()},
            "context_for_llm": "\n".join(lines),  # tokens only, safe for LLM
        }
    }
